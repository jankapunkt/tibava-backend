    ...
    def post(self, request):
        try:
            ...
            if "plugin_item_ref_list" not in data:
                return JsonResponse({"status": "error", "type": "missing_values_plugin_item_ref_list"})
            ...
            for plugin_item_ref in plugin_item_ref_list:
                cluster_items = ClusterItem.objects.filter(plugin_item_ref=plugin_item_ref) # TODO change filter to "get" as soon as old clusters are deleted
                if (len(cluster_items) > 1):
                    cluster_items = [f for f in cluster_items if f.cluster_timeline_item.cluster_id.hex == data.get("cluster_id")]
                assert len(cluster_items) == 1, f"still more than one cluster_item: {cluster_items} \n {data}"
