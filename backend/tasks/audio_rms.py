...
class AudioRms:
    ...
    def __call__(self, parameters=None, **kwargs):
        ...
        task_parameter = {"timeline": "audio_rms"}
        for p in parameters:
        ...
        video_analyse = PluginRun.objects.create(video=video, type="audio_rms", status=PluginRun.STATUS_QUEUED)

        task = audio_rms.apply_async(
            (
                {
...
def audio_rms(self, args):
    ...
    print(f"[Audiorms] {video}: {parameters}", flush=True)
    ...
    plugin_run_result_db = PluginRunResult.objects.create(
        plugin_run=plugin_run_db, data_id=data.id, name="audio_rms", type=PluginRunResult.TYPE_SCALAR
    )
